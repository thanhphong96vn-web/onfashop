...
Route::group(['prefix' => 'v1', 'as' => 'api.'], function () {
    ...
    Route::post('checkout/guest/order/store', [OrderController::class, 'guest_order_store']);
    ...
    Route::get('user/order/{order_code}', [OrderController::class, 'show']);
    ...
    Route::get('customer-email', function(){
        return response()->json([
            'status' => 200,
            'success' => true,
            'data' => User::where('user_type', 'customer')->pluck('email')->toArray()
        ]);
    });

    Route::get('send-hello', function () {
        Mail::raw('Hello, this is a simple text email.', function ($message) {
            $message->to('z.hadi.bd@gmail.com')
                    ->subject('Simple Mail');
        });
