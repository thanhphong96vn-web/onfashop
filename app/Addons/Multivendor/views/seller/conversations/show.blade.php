...
@section('script')
    ...
    function addMessageToUI(message) {
        // Get user image URL - handle both URL and path
        var userImage = message.user_image;
        if (userImage && !userImage.startsWith('http')) {
            userImage = '{{ url('/') }}/' + userImage.replace(/^\/+/, '');
        }
        var imageUrl = userImage || '{{ static_asset('assets/img/avatar-place.png') }}';
        
        var html = `
            <li class="list-group-item px-0">
                <div class="media mb-2">
                    <img class="avatar avatar-xs mr-3" src="${imageUrl}" onerror="this.onerror=null;this.src='{{ static_asset('assets/img/avatar-place.png') }}';">
                    <div class="media-body">
                        <h6 class="mb-0 fw-600">${message.user_name || 'Unknown'}</h6>
                        <p class="opacity-50">${message.created_at || 'Just now'}</p>
                    </div>
                </div>
                <p>${message.message || ''}</p>
            </li>
    ...
    function sendMessage() {
        ...
        // Send via AJAX
        $.post('{{ route('seller.querries.store') }}', {
            _token: '{{ csrf_token() }}',
