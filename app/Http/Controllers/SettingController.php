...
class SettingController extends Controller
{
    ...
    // Method removed - license check no longer needed
    ...
    public function update(Request $request)
    {
        // return $request->all();
        foreach ($request->types as $key => $type) {
            if($type == "customer_otp_with" && $request->customer_otp_with == 'email' ){
                if(!smtp_is_ok()) {
                    flash(translate("You must configure SMTP settings before enabling customer OTP."))->error();
                    return back();
                }
            }
            if ($type == 'timezone') {
    ...
    {
        return view('backend.settings.features_control');
    }
}
