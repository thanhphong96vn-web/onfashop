...
class OrderController extends Controller
{
    ...
    {
        $query = CombinedOrder::with(['user', 'orders.orderDetails.variation.product', 'orders.orderDetails.variation.combinations', 'orders.shop'])
        ...
        if ($request->has('delivery_status') && $request->delivery_status) {
            ...
            // Special case for unpaid status (payment_status filter)
            if ($deliveryStatus === 'unpaid') {
                $query->whereHas('orders', function ($q) {
                    $q->where('payment_status', 'unpaid');
                });
            } 
            // Special case for refund requested
            elseif ($deliveryStatus === 'refund_requested') {
                $query->whereHas('orders', function ($q) {
                    $q->whereHas('refundRequests');
                });
            }
            // Normal delivery status filter
            else {
                $query->whereHas('orders', function ($q) use ($deliveryStatus) {
        ...
        if ($request->has('search') && $request->search) {
            ...
            $query->where(function ($q) use ($search) {
                ...
                $q->where('code', 'like', "%{$search}%")
                    ...
                    ->orWhereHas('orders.orderDetails.variation.product', function ($q2) use ($search) {
                        $q2->where('name', 'like', "%{$search}%");
                    })
                    // Search by shop name
                    ->orWhereHas('orders.shop', function ($q3) use ($search) {
        ...
        // Get counts for tabs
        $counts = [
            'all' => CombinedOrder::where('user_id', auth()->id())->count(),
            'unpaid' => CombinedOrder::where('user_id', auth()->id())
                ->whereHas('orders', function ($q) {
                    $q->where('payment_status', 'unpaid');
                })->count(),
            'pending' => CombinedOrder::where('user_id', auth()->id())
                ->whereHas('orders', function ($q) {
                    $q->where('delivery_status', 'order_placed');
                })->count(),
            'shipping' => CombinedOrder::where('user_id', auth()->id())
                ->whereHas('orders', function ($q) {
                    $q->where('delivery_status', 'on_delivery');
                })->count(),
            'delivered' => CombinedOrder::where('user_id', auth()->id())
                ->whereHas('orders', function ($q) {
                    $q->where('delivery_status', 'delivered');
                })->count(),
            'cancelled' => CombinedOrder::where('user_id', auth()->id())
                ->whereHas('orders', function ($q) {
                    $q->where('delivery_status', 'cancelled');
                })->count(),
            'refund' => CombinedOrder::where('user_id', auth()->id())
                ->whereHas('orders', function ($q) {
                    $q->whereHas('refundRequests');
                })->count(),
        ];
    ...
    public function cancel($order_id)
    {
        ...
        if ($order->delivery_status == 'order_placed' && $order->payment_status == 'unpaid') {
            ...
            foreach ($order->orderDetails as $orderDetail) {
                try {
                    // Update category sales_amount - divide evenly among all categories
                    $categories = $orderDetail->product->categories;
                    $categoryCount = $categories->count();
                    if ($categoryCount > 0) {
                        $amountPerCategory = $orderDetail->total / $categoryCount;
                        foreach ($categories as $category) {
                            $category->sales_amount -= $amountPerCategory;
                            $category->save();
                        }
                    }

                    // Update brand sales_amount (only one brand per product, so no division needed)
