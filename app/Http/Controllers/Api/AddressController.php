...
class AddressController extends Controller
{
    public function addresses(Request $request)
    {
        ...
        if ($request->filled('temp_user')) {
            return new AddressCollection(
                Address::where('temp_user_id', $request->temp_user)->latest()->get()
            );
        }
    ...
    public function createShippingAddress(Request $request)
    {
        if($request->guest_user_id){
            $shipping_count = Address::where('temp_user_id', $request->guest_user_id)->where('default_shipping', 1)->count();
            $billing_count = Address::where('temp_user_id', $request->guest_user_id)->where('default_billing', 1)->count();
        }else {
        ...
        $address = new Address;
        if($request->guest_user_id){
            $address->temp_user_id = $request->guest_user_id;
        }else {
    ...
    public function updateShippingAddress(Request $request)
    {
        ...
        if($request->guest_user_id){
            if($request->guest_user_id != $address->temp_user_id) {
                 return response()->json(null, 401);
            }
        }else{
        ...
        if($user = auth('api')->user()){
            $data =  Address::where('user_id', $user->id)->latest()->get();
        }else {
            $data = Address::where('temp_user_id', $request->guest_user_id)->latest()->get();
        }
