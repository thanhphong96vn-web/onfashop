    ...
    public function toArray($request)
    {
        return [
            'data' => $this->collection->map(function($data) {
                // Use products_count from withCount if available, otherwise calculate
                if (isset($data->products_count)) {
                    $products_count = $data->products_count;
                } else {
                    $products_count = \App\Models\Product::where('shop_id', $data->id)
                        ->where('published', 1)
                        ->where('approved', 1)
                        ->count();
                }
                ...
                return [
                    ...
                    'since' => $data->created_at ? $data->created_at->format('d M, Y') : '',
                    'isVarified' => $data->verification_status ==1 ? true : false,
                ];
            })
        ];
    }
