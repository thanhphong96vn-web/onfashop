...
if (!function_exists('convert_to_usd')) {
    function convert_to_usd($amount)
    {
        ...
        // }
        $result =  floatval($amount) / floatval($defaultCurrency->exchange_rate);
        return $result;
    }
}
    ...
    function format_price($price, $show_tag = false)
    {
        ...
        if ($show_tag) {
            if ($price < 1000000) {
                ...
                $fomated_price = number_format($price, get_setting('no_of_decimals'));
                // Use normal symbol format for amounts less than million
                ...
                // Anything less than a billion - format as "triệu" (million)
                $million_value = $price / 1000000;
                $decimals = get_setting('no_of_decimals');
                // Use at least 1 decimal when formatting as "triệu" to avoid rounding errors
                $decimals = max(1, $decimals);
                $formatted = number_format($million_value, $decimals, '.', '');
                // Remove trailing zeros if decimals exist
                if ($decimals > 0) {
                    $formatted = rtrim(rtrim($formatted, '0'), '.');
                }
                // Always add space before currency symbol when using "triệu"
                return $formatted . ' triệu đồng';
            } else {
                // At least a billion - format as "tỷ" (billion)
                $billion_value = $price / 1000000000;
                $decimals = get_setting('no_of_decimals');
                // Use at least 1 decimal when formatting as "tỷ" to avoid rounding errors
                $decimals = max(1, $decimals);
                $formatted = number_format($billion_value, $decimals, '.', '');
                // Remove trailing zeros if decimals exist
                if ($decimals > 0) {
                    $formatted = rtrim(rtrim($formatted, '0'), '.');
                }
                // Always add space before currency symbol when using "tỷ"
                return $formatted . ' tỷ ' . currency_symbol();
            }
        }
...
//converts dollar to default currancy
if (!function_exists('convert_price2')) {
    function convert_price2($price)
    {
        $defaultCurrency = get_system_default_currency();



        if ($defaultCurrency && isset($defaultCurrency->exchange_rate) && $price !== null) {
            // $exchangeRate = floatval(Session::get('currency_exchange_rate', 1)); 
            // $business_settings = Setting::where('type', 'system_default_currency')->first();
            // Log::info($defaultCurrency);
            $price = floatval($price) / 1;
            $price = $price * $defaultCurrency->exchange_rate;
        }
...
/**
 * Get Vietnamese translation for order delivery status
 */
if (!function_exists('get_order_status_vn')) {
    function get_order_status_vn($status)
    {
        $statusMap = [
...
if (!function_exists('get_setting')) {
    function get_setting($key, $default = null)
    {
        try {
            ...
            // Return default value if database is not available (e.g., during package discovery)
            return $default;
        }
    }
}
...
if (!function_exists('smtp_is_ok')) {
    function smtp_is_ok()
    {
       $values = [
        ...
        return collect($values)->every(fn($val) => !empty($val));
    }
}