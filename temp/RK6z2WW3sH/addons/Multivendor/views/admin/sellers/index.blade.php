                ...
                <div class="col-md-3">
                    <div class="form-group mb-0">
                        <input type="text" class="form-control" id="search" name="search"
                            ...
                            placeholder="{{ translate('Type name or phone & Enter') }}">
                    </div>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <img src="{{ uploaded_asset($shop->user->avatar ?? null) }}"class="size-50px rounded-circle mr-2"
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <img src="{{ uploaded_asset($shop->logo) }}"class="size-50px rounded-circle mr-2"
                                            onerror="this.onerror=null;this.src='{{ static_asset('/assets/img/placeholder.jpg') }}';" />
                                        <span class="flex-grow-1 minw-0">
                                            <div class="text-truncate fs-12 fw-600">{{ $shop->name }}</div>
                                            <div class="text-truncate fs-12">{{ translate('Phone') . ': ' . $shop->phone }}
                                            </div>
                                            <div class="text-truncate fs-12">
                                                {{ translate('Total products') . ': ' . $shop->products_count }}</div>
                                        </span>
                                    </div>
                                </td>
                                <td class="text-center">
                                    @if ($shop->verification_status == 1 && $shop->verification_info != null)
                                        <i class="las la-2x la-check-circle text-info"></i>
                                    @else
                                        <span class="badge badge-inline badge-warning">Not verified</span>
                                    @endif
                                </td>
                                <td>
                                    ...
                                    @else
                                        <span
                                            class="badge badge-inline badge-danger">{{ translate('No active Package') }}</span>
                                    @endif
                                </td>
                                <td>
                                    @if ($shop->current_balance == 0)
                                        ...
                                        <span
                                            class="fs-16 fw-700 text-secondary">{{ format_price($shop->current_balance) }}</span>
                                    @elseif($shop->current_balance >= 0)
                                        <div>{{ translate('Due to seller') }}:</div>
                                        <span
                                            class="fs-16 fw-700 text-danger">{{ format_price($shop->current_balance) }}</span>
                                    @else
                                        <div>{{ translate('Due from seller') }}:</div>
                                        <span
                                            class="fs-16 fw-700 text-success">{{ format_price(abs($shop->current_balance)) }}</span>
                                    @endif
                                </td>
                                <td>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input onchange="seller_approval(this)" value="{{ $shop->id }}" type="checkbox"
                                            @if ($shop->approval == 1) checked @endif>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input onchange="shop_publish(this)" value="{{ $shop->id }}" type="checkbox"
                                            @if ($shop->published == 1) checked @endif>
                                        <span class="slider round"></span>
                                    </label>
                                </td>
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                            <a href="#" onclick="show_seller_profile('{{ $shop->user->id }}');"
                                                class="dropdown-item">
                                                {{ translate('Profile') }}
                                            </a>
                                            <a href="{{ route('admin.show_verification_request', $shop->id) }}"
                                                class="dropdown-item">
                                                {{ translate('Verification Info') }}
                                            </a>

                                            <a href="#"
                                                onclick="show_seller_payment_modal('{{ $shop->user->id }}');"
                                                class="dropdown-item">
                                            ...
                                            <a href="{{ route('admin.seller.edit', encrypt($shop->user->id)) }}"
                                                class="dropdown-item">
                                                {{ translate('Edit') }}
                                            </a>
                                            <a href="#" class="dropdown-item confirm-delete"
                                                data-href="{{ route('admin.seller.destroy', $shop->user->id) }}"
                                                class="">
                                                {{ translate('Delete') }}
                                            </a>
                                        </div>
