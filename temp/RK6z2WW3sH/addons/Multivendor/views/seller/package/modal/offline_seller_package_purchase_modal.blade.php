<form class="" action="{{ route('seller.packages.purchase') }}" method="post" enctype="multipart/form-data">
    @csrf
    <input type="hidden" name="seller_package_id" value="{{ $package_id }}">
            ...
            @foreach ($manual_payment_methods as $method)
                <div class="col-6 col-md-4">
                    <label class="aiz-megabox d-block mb-3">
                        <input value="{{ $method->heading }}" id="payment_option" type="radio"
                            data-method-id="{{ $method->id }}" name="payment_option"
                            onchange="toggleManualPaymentData({{ $method->id }})" checked>
        ...
        <div id="manual_payment_data">
            ...
            <div class="card mb-3 p-3">
                ...
                <div class="form-group row">
                    <label class="col-md-3 col-form-label">{{ translate('Photo') }}<span
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        var id = $('input[name="payment_option"]:last').attr('data-method-id');
