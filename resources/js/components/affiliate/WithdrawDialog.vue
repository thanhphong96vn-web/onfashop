        ...
        <div class="white pa-5 rounded">
            <v-form lazy-validation v-on:submit.prevent>
                ...
                <v-text-field
                    variant="plain"
                    class="text-field"
                    :placeholder="$t('withdrawal_amount')"
                    type="Number"
...
export default {
    ...
    methods: {
        ...
        async withdrawalRequest() {
            ...
            if(this.getAffiliateBalance < this.requestedAmount){
                this.snack({
                message: this.$i18n.t("you can't request for withdraw more than your affiliate balance"),
                color: "red",
            });
            return;
            }
            ...
            const res = await this.call_api(
                "post",
                `user/affiliate/withdraw-request`,
                amount
            );
            if(res.data.success){
