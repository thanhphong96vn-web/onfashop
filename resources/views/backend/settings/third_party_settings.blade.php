        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h3 class="mb-0 h6">{{ translate('Google reCAPTCHA 3 Setting') }}</h3>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('google_recaptcha.update') }}" method="POST">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="CAPTCHA_KEY"
                                    value="{{ config('services.recaptcha.site') }}" placeholder="{{ translate('Site KEY') }}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="CAPTCHA_SECRET">
                            <div class="col-md-4">
                                <label class="control-label">{{ translate('Secret KEY') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="CAPTCHA_SECRET"
                                    value="{{ config('services.recaptcha.secret') }}" placeholder="{{ translate('Secret KEY') }}"
                                    required>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select class="form-control aiz-selectpicker" name="RECAPTCHA_SCORE_THRESHOLD"
                                    id="accept-v3-score" data-live-search="true">
                                    <option value="">{{ translate('Select Score') }}</option>
                                    <option value="0.3"
                                        {{ env('RECAPTCHA_SCORE_THRESHOLD') == '0.3' ? 'selected' : '' }}>More than or
                                        equal to 0.3</option>
                                    <option value="0.5"
                                        {{ env('RECAPTCHA_SCORE_THRESHOLD') == '0.5' ? 'selected' : '' }}>More than or
                                        equal to 0.5</option>
                                    <option value="0.7"
                                        {{ env('RECAPTCHA_SCORE_THRESHOLD') == '0.7' ? 'selected' : '' }}>More than or
                                        equal to 0.7</option>
                                    <option value="0.9"
                                        {{ env('RECAPTCHA_SCORE_THRESHOLD') == '0.9' ? 'selected' : '' }}>More than or
                                        equal to 0.9</option>
                                </select>
                                <small
                                    class="text-muted">{{ translate('Google reCAPTCHA v3 returns a score ranging from 0.0 to 1.0 that indicates the likelihood a request is made by a human or a bot.') }}</small>
                            </div>
                        </div>
                ...
                <div class="card-body">
                    <ul class="list-group">
                        <li class="list-group-item">
                            1. Score: 0.0 - 0.3 : <strong>Very likely a bot</strong> — Recommended Action: Block the request
                            or require additional verification.
                        </li>
                        <li class="list-group-item">
                            2. Score: 0.3 - 0.5 : <strong>Suspicious activity</strong> — Recommended Action: Might want to
                            require additional verification.
                        </li>
                        <li class="list-group-item">
                            3. Score: 0.5 - 0.7 : <strong>Possibly human</strong> — Recommended Action: Could be legitimate
                            traffic.
                        </li>
                        <li class="list-group-item">
                            4. Score: 0.7 - 0.9 : <strong>Likely human</strong> — Recommended Action: Probably safe to
                            allow.
                        </li>
                        <li class="list-group-item">
                            5. Score: 0.9 - 1.0 : <strong>Very likely human</strong> — Recommended Action: Definitely safe
                            to allow.
                        </li>
                        <li class="list-group-item">
                            6. If Google reCAPTCHA v3 credentials have not yet been created, please register your site by
                            visiting
                            <a href="https://www.google.com/recaptcha/admin/create" target="_blank">this link</a> and
                            ...
                            <div class="row">
                                @php
                                    $settings = [
                                        ...
                                        'recaptcha_shop_register' => 'Shop Registration',
                                        'recaptcha_affiliate_register' => 'Affiliate Registration',
                                        'recaptcha_track_order' => 'Track Order Page',
                                    ];
                                    ...
                                    <div class="col-lg-2 col-md-3 col-sm-4 col-xs-6">
                                        <div class="p-2 border mt-1 mb-2">
                                            ...
                                            <label class="aiz-switch aiz-switch-success">
                                                <input type="checkbox" {{ get_setting('google_recaptcha') != 1 ? 'disabled' : '' }}
                                                    onchange="triggerConfirmation(this, '{{ $key }}', '{{ $label }}')"
                                                    {{ get_setting($key) == 1 ? (get_setting('google_recaptcha') != 1 ? 'disabled' : 'checked') : '' }}>
                                                <span class="slider round"></span>
                                            </label>
                                        </div>
                                    </div>
                    ...
                    <div class="card shadow-none bg-light">
                        ...
                        <div class="card-body">
                            <ul class="list-group mar-no">
                                <li class="list-group-item text-dark">1. {{ translate('Login into your facebook page') }}
                                </li>
                                ...
                                <li class="list-group-item text-dark">6. {{ translate('Set your website domain name') }}.
                                </li>
                            </ul>
                        </div>
                    </div>
                ...
                <div class="modal-body text-center">
                    ...
                    <div>
                        ...
                        <button type="button" class="btn btn-warning rounded-2 mt-2 fs-13 fw-700 w-250px"
                            onclick="confirmSettingChange()">Confirm</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        function toggleRecaptchaChildren(isEnabled) {
            $('input[type="checkbox"]').each(function() {
                if ($(this).attr('onchange')?.includes('triggerConfirmation')) {
                    ...
                    $(this).closest('.border').css('opacity', isEnabled ? 1 :
                    0.5); // Optional: Visual cue
                }
            });
        }
    });
